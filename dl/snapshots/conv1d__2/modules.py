...
class Conv1DBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn(x)
        x = self.relu(x)
        if self.residual and x.shape == res.shape:
            x = x + res
        x = self.do(x)
        return x
...
class Conv1dClassifier(nn.Module):
    def __init__(self, in_channels=248, num_class=4):
        ...
        self.conv4 = Conv1DBlock(256, 256, kernel_size=16, stride=1, padding=1, drouput=0)
        # spacial attention
...
class FitConv1dClassifier(nn.Module):
    def __init__(self, in_channels=248, num_class=4):
        ...
        self.conv1 = Conv1DBlock(in_channels, 256, kernel_size=16, stride=1, padding=1)
        # 256x195
        self.conv2 = Conv1DBlock(256, 256, kernel_size=16, stride=2, padding=1)
        # 256x98
        self.conv3 = Conv1DBlock(256, 256, kernel_size=16, stride=2, padding=1)
        # 256x49
        self.conv4 = Conv1DBlock(256, 256, kernel_size=16, stride=1, padding=1, drouput=0)
        # spacial attention
        self.classifier = nn.Linear(256*49, num_class)
        self.noise_layer = GaussianNoise(0.1, True)
