...
class Conv1dClassifier(nn.Module):
    def __init__(self, in_channels=248, num_class=4):
        ...
        self.conv1 = Conv1DBlock(in_channels, 256, kernel_size=self.kernel_size, stride=1, padding=1)
        self.conv2 = Conv1DBlock(256, 256, kernel_size=self.kernel_size, stride=1, padding=1)
        self.conv3 = Conv1DBlock(256, 256, kernel_size=self.kernel_size, stride=1, padding=1)
...
class FitConv1dClassifier(nn.Module):
    def __init__(self, in_channels=248, num_class=4):
        ...
        self.droupout = 0.2
        # 248x195
        self.conv1 = Conv1DBlock(in_channels, self.dim, kernel_size=self.kernel_size, stride=1, padding=1, drouput=self.droupout)
        # 256x195
        self.conv2 = Conv1DBlock(self.dim, self.dim, kernel_size=self.kernel_size, stride=1, padding=1, drouput=self.droupout)
        # 256x98
        self.conv3 = Conv1DBlock(self.dim, self.dim, kernel_size=self.kernel_size, stride=1, padding=1, drouput=self.droupout)
        # 256x49
        ...
        # )
        self.noise_layer = GaussianNoise(1, False)
